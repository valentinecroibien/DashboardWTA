...
### Choix de la joueuse
```{r}
joueuses <- c("Justine Henin", "Serena Williams")
selectInput("joueuse", label = "Sélection :",
            choices = joueuses, selected = "Justine Henin")
```

row 
-----------------------------------------------------------------------
### Pour les années 2000 à 2010

```{r}

titre <- "Classement féminin de tennis : classement WTA"

valueBox(titre, icon = "fa-trophy", color = "lightblue")
```
...
```{r}
id_wiki <- wta_players$wikidata_id[which(wta_players$player_id == id_joueuse)]

url_wikidata <- paste("https://www.wikidata.org/wiki/", id_wiki, sep="")

page_wiki <- xml2::read_html(url_wikidata, encoding="UTF-8")

imgs <- html_nodes(page_wiki, "img")%>%
  html_attr('src')

image_profil <- str_sub(imgs[1], 3,-1)

download.file(image_profil, "profil.jpg", mode = "wb")
```
...
<p> <a>Nationalité :</a> `r nat_joueuse` </p>
...
valueBox(value = med,icon = "fas fa-medal",caption = "Nombre de médailles olympiques", color = "lightgreen")
```

### Nombre de titres

```{r}
valueBox(value = titres,icon = "fas fa-trophy",caption = "Nombre de titres", color = "lightblue")
```
...
```{r}
valueBox(value = gains, icon = "fas fa-dollar-sign",caption = "Gains en tournois", color = "lightpink")
```
